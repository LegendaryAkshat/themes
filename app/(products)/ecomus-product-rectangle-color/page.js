...
export default function Page() {
  ...
  const getTextColor = (colorName, isSelected) => {
    if (!isSelected) return "#000";
    return colorName === "White" ? "#000" : "#fff";
  };
                  ...
                  return (
                    <button
                      ...
                      className={`px-6 py-3 rounded-lg border-2 transition-all font-semibold ${
                        isSelected ? colors.buttons.color.selected : colors.buttons.color.unselected
                      }`}
                      style={{
                        backgroundColor: isSelected ? color.value : "transparent",
                        color: getTextColor(color.name, isSelected)
                      }}
                    >
                      {color.name}
                    </button>
