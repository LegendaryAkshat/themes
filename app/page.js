...
const pageConfig = {
  ...
  colors: {
    ...
    gradients: {
      hover: "hover:bg-gradient-to-br hover:from-blue-50 hover:to-indigo-50",
      overlay: "bg-gradient-to-r from-blue-500/0 to-indigo-500/0 group-hover:from-blue-500/5 group-hover:to-indigo-500/5"
    }
  },
  ...
  header: {
    title: "Theme Gallery",
    subtitle: "Browse and preview all sections",
    icon: {
  ...
  footer: {
    text: "Click on any page to preview and inspect the design"
  },
  
  // Page Categories (Edit categories and pages here!)
  pageCategories: [
    {
    ...
    },
    {
      ...
      pages: [
        ...
        { name: "Vercel Store Account Addresses", path: "/vercel-store-addresses" },
...
export default function HomePage() {
  const { colors, header, stats, footer, pageCategories } = pageConfig;
  const totalPages = pageCategories.reduce((sum, category) => sum + category.pages.length, 0);

  return (
    <div className={`min-h-screen ${colors.background}`}>
      {/* Header */}
      <header className={`${colors.header.background} ${colors.header.border} border-b sticky top-0 z-50`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className={`p-2 ${header.icon.background} rounded-lg`}>
                <Home className="w-6 h-6 text-white" />
              </div>
              <div>
                <h1 className={`text-2xl font-bold ${colors.text.accent}`}>
                  {header.title}
                </h1>
                <p className={`text-sm ${colors.text.secondary}`}>{header.subtitle}</p>
              </div>
            </div>
            <div className="text-right">
              <div className={`text-2xl font-bold ${colors.text.primary}`}>{totalPages}</div>
              <div className={`text-xs ${colors.text.secondary}`}>Total Pages</div>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        {stats.showStats && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
            <div className={`${colors.card.background} rounded-xl p-6 shadow-sm ${colors.card.border} border`}>
              <div className="flex items-center justify-between">
                <div>
                  <p className={`text-sm ${colors.text.secondary} mb-1`}>Total Categories</p>
                  <p className={`text-3xl font-bold ${colors.text.primary}`}>{pageCategories.length}</p>
                </div>
                <div className={`p-3 ${colors.stats.blue} rounded-lg`}>
            ...
            <div className={`${colors.card.background} rounded-xl p-6 shadow-sm ${colors.card.border} border`}>
              <div className="flex items-center justify-between">
                <div>
                  <p className={`text-sm ${colors.text.secondary} mb-1`}>Total Pages</p>
                  <p className={`text-3xl font-bold ${colors.text.primary}`}>{totalPages}</p>
                </div>
                <div className={`p-3 ${colors.stats.green} rounded-lg`}>
            ...
            <div className={`${colors.card.background} rounded-xl p-6 shadow-sm ${colors.card.border} border`}>
              <div className="flex items-center justify-between">
                <div>
                  <p className={`text-sm ${colors.text.secondary} mb-1`}>Ready to Use</p>
                  <p className={`text-3xl font-bold ${colors.text.primary}`}>{stats.readyToUse}</p>
                </div>
                <div className={`p-3 ${colors.stats.purple} rounded-lg`}>
        ...
        <div className="space-y-8">
          {pageCategories.map((category, categoryIndex) => {
            ...
            return (
              <div key={categoryIndex} className={`${colors.card.background} rounded-2xl shadow-sm ${colors.card.border} border overflow-hidden`}>
                {/* Category Header */}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                    {category.pages.map((page, pageIndex) => (
                      <Link
                        key={pageIndex}
                        href={page.path}
                        className={`group relative p-4 rounded-lg ${colors.card.border} border ${colors.card.hover} transition-all duration-200 ${colors.card.background} ${colors.gradients.hover}`}
                      >
                        <div className="flex items-center justify-between mb-2">
                          <h3 className={`font-semibold ${colors.text.primary} group-hover:text-blue-600 transition-colors`}>
                            {page.name}
                          </h3>
                          <ArrowRight className={`w-4 h-4 ${colors.text.secondary} group-hover:text-blue-600 group-hover:translate-x-1 transition-all`} />
                        </div>
                        <p className={`text-xs ${colors.text.secondary} group-hover:text-gray-600`}>
                          {page.path}
                        </p>
                        <div className={`absolute inset-0 rounded-lg ${colors.gradients.overlay} transition-all`} />
                      </Link>
                    ))}
                  </div>
                </div>
